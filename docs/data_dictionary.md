...
| `SP_DIABETES`, `SP_CHF`, `SP_CNCR`, `SP_COPD`, etc. | Chronic condition indicator flags. **Coding convention (1 = has condition, 2 = does not) is an assumption, not yet verified against the DE-SynPUF codebook.** | **Open** |
...
| `CLM_UTLZTN_DAY_CNT` | CMS-supplied length-of-stay in days. **Not yet cross-checked against `NCH_BENE_DSCHRG_DT − CLM_ADMSN_DT` raw date arithmetic — this is the second open verification item.** | **Open** |
